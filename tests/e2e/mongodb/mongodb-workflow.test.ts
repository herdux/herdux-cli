...
const TEST_DB = "herdux_mongodb_testdb";
const PROJECT_ROOT = resolve(__dirname, "..", "..", "..");
const BACKUP_DIR = resolve(PROJECT_ROOT, "tests", "e2e", "mongodb", "backups");
const TMP_HOME = resolve(PROJECT_ROOT, "tests", "e2e", "mongodb", ".tmp-home");

// Track backup file path for restore tests
let archiveBackupPath: string;
...
describe("E2E: MongoDB Full Workflow", () => {
  // --- Doctor ---

  describe("doctor", () => {
    it("should verify system health for MongoDB", async () => {
      const result = await runCli("doctor");

      expect(result.exitCode).toBe(0);
      expect(result.output).toContain("mongosh");
      expect(result.output).toContain("mongodump");
    });
  });
  ...
  // --- Backup (archive / mongodump default) ---

  describe("backup", () => {
    it("should create a backup in archive format", async () => {
      const result = await runCli("backup", TEST_DB, "--output", BACKUP_DIR);

      expect(result.exitCode).toBe(0);
      expect(result.output).toMatch(/backup saved/i);

      const files = readdirSync(BACKUP_DIR);
      const archiveFile = files.find((f) => f.endsWith(".mongodump"));
      expect(archiveFile).toBeDefined();
      archiveBackupPath = resolve(BACKUP_DIR, archiveFile!);
    });

    it("should reject --format plain with a clear error", async () => {
      ...
      expect(result.output).toContain('does not support "plain" format');
    });
  });

  // --- Inspect (.mongodump backup) ---

  describe("inspect (.mongodump backup)", () => {
    it("should inspect the .mongodump backup file without a live connection", async () => {
      const result = await runCli("inspect", archiveBackupPath);
  ...
  describe("restore", () => {
    it("should restore the database from .mongodump backup", async () => {
      const result = await runCli(
        "restore",
        archiveBackupPath,
        "--db",
